...
router.post("/password-reset/:userId/:token", async (req, res) => {
  try {
    const user = await Admin.findById({_id : new ObjectID(req.params.userId)});
...
await Admin.findOneAndUpdate({_id : new ObjectID(user._id)}, { password: hash}, { new: true }, (err,data) => {
  if(err) console.log(err);
  console.log(data)
}).clone();
    await tok.delete();
