...
router.route("/register").post(
  (req, res, next) => {
    Teacher.findOne({ fullName: req.body.fullName }, (err, user) => {
      if (err) {
        next(err);
      } else if (user) {
  ...
  (req, res, next) => {
    ...
    Teacher.findOne({ fullName: userAuth }, (err, user) => {
      if (err) console.log(err);
...
router.post("/password-reset/:userId/:token", async (req, res) => {
  try {
    const user = await Teacher.findById({_id : new ObjectID(req.params.userId)});
...
await Teacher.findOneAndUpdate({_id : new ObjectID(user._id)}, { password: hash}, { new: true }, (err,data) => {
  if(err) console.log(err);
  console.log(data)
}).clone();
    await tok.delete();
...
passport.deserializeUser((id, done) => {
    Teacher.findOne({ _id: new ObjectID(id) }, (err, doc) => {
    done(null, doc);
  });
});
